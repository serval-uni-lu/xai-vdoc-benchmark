...
def tv_norm(image, beta=2):
    ...
    image = image[:, 0, :, :]
    a = torch.mean(
        torch.abs((image[:, :-1, :] - image[:, 1:, :]).view(image.shape[0], -1)).pow(
            beta
        ),
        dim=1,
    )
...
def bilateral_tv_norm(image, mask, tv_beta=2, sigma=1):
    ...
    mask_ = mask[:, 0, :]
    a = torch.mean(
        torch.abs((mask_[:, :-1, :] - mask_[:, 1:, :]).view(mask.shape[0], -1)).pow(
            tv_beta
        ),
        dim=1,
    )
    ...
    bil_a = torch.mean(
        torch.exp(
            -((image[:, :, :-1, :] - image[:, :, 1:, :]).mean(dim=1) ** 2) / sigma
        ).view(mask.shape[0], -1)
        * torch.abs(
            (up_mask_[:, :, :-1, :] - up_mask_[:, :, 1:, :]).view(up_mask_.shape[0], -1)
        ).pow(tv_beta),
        dim=1,
    )
    bil_b = torch.mean(
        torch.exp(-((image[:, :, :, :-1] - image[:, :, :, 1:]) ** 2) / sigma)
        .mean(dim=1)
        .view(mask.shape[0], -1)
        * torch.abs(
            (up_mask_[:, :, :, :-1] - up_mask_[:, :, :, 1:]).view(up_mask_.shape[0], -1)
...
def upscale(
    masks: torch.Tensor, images, mode: str = "bilinear", align_corners: bool = False
):
    """
...
def interval_score(
    ...
    if model_name == "llava" or model_name == "llava_next" or model_name == "mgm":
        ...
        intervals = (
            torch.linspace(1 / num_iter, 1, num_iter, requires_grad=False)
            .cuda()
            .view(-1, 1, 1, 1)
        )
        ...
        losses = torch.tensor(0.0).to(input_ids.device)
        ...
        if noise:
            local_images = [
                local_image + torch.randn_like(local_image) * 0.2
                for local_image in local_images
            ]

        local_images = [local_image.squeeze(0) for local_image in local_images]
        local_images = [
            [local_images[i][j] for i in range(len(local_images))]
...
def line_search(
    ...
    with torch.no_grad():
        ...
        up_masks = upscale(masks.view(-1, *masks.shape[mod:])).view(
            -1, *masks.shape[1:mod], 1, upscale.out_size, upscale.out_size
        )
        ...
        while True:
            ...
            new_masks = torch.clamp(
                masks[indices]
                - alphas[indices].view(-1, *(1,) * mod, 1, 1) * total_grads[indices],
                0,
                1,
            )
            up_masks = upscale(new_masks.view(-1, *masks.shape[mod:])).view(
                -1, *masks.shape[1:mod], 1, upscale.out_size, upscale.out_size
            )
            ...
            indices[indices] = (
                losses > base_losses[indices] + alphas[indices] * t[indices]
            )
...
def metric(
    ...
    with torch.no_grad():
        ...
        for pixels in range(0, num_pixels, step):
            ...
            del_scores += (
                outputs.cpu() * step
                if pixels + step < num_pixels
                else num_pixels - pixels
            )
...
def find_keywords(
    ...
    condition = (
        (torch.log(probs) - torch.log(probs_blur) > 1.0)
        & (probs >= 0.0)
