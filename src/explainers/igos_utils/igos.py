...
from .utils import get_initial, get_kernel_size
...
def gen_explanations_qwenvl(
    model, processor, image, text_prompt, tokenizer, positions=None, select_word_id=None
):
    """_summary_
...
def gen_explanations_internvl(
    ...
    generated_ids = torch.cat(
        [inputs["input_ids"], y if y.dim() == 2 else y.unsqueeze(0)], dim=1
    ).to(model.device)
...
def iGOS_pp(
    ...
    **kwargs,
    ...
    def regularization_loss(image, masks):
        return (
            L1 * torch.mean(torch.abs(1 - masks).view(masks.shape[0], -1), dim=1),
    ...
    masks_ins = torch.ones(
        (image.shape[0], 1, size, size), dtype=torch.float32, device="cuda"
    )
    ...
    for i in range(iterations):
        ...
        loss_comb_del = integrated_gradient(
            model,
            inputs,
            generated_ids,
            image,
        ...
        total_grads1 -= (
            masks_del.grad.clone()
        )  # Negative because insertion loss is 1 - score.
        total_grads2 -= masks_ins.grad.clone()
        masks_del.grad.zero_()
        masks_ins.grad.zero_()
        ...
        if opt == "LS":
            ...
            total_grads = torch.cat(
                (total_grads1.unsqueeze(1), total_grads2.unsqueeze(1)), 1
            )
