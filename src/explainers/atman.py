...
class AtManExplainer(BaseExplainer):
    def __init__(self, 
                 ...
                 batch_size: int = 16,
                 conceptual_threshold: Optional[float] = None):
        """
        Args:
    ...
    def attribute(self, 
        ...
        with self.manage_explainability_state():
            for i in range(0, num_perturbations, self.batch_size):
                ...
                suppress_bool_mask = torch.zeros((current_b_size, base_input_ids.shape[1]), dtype=torch.bool, device=base_input_ids.device)
